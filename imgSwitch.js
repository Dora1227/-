...
imgSwitch = function(container,options){
	...
	this.Auto = this.options.Auto;//是否自动播放
	this.Pause = this.options.Pause;//暂停时间
	this.Type = this.options.Type;//播放类型
	this.Navigate = this.options.Navigate;//导航，有三种，分别为no(无),numberic(数字),picture(图片)
	this.Width = this.options.Width;//宽
	this.Height = this.options.Height;//高
	this.Speed = this.options.Speed=="fast"?10:this.options.Speed=="slow"?20:this.options.Speed.match(/\D/g)?15:this.options.Speed;//切换速度
	this.PicturePosition = this.options.PicturePosition;//图片位置
	this.NavigatePlace = this.options.NavigatePlace;//导航位置
	this.arrImgs = this._container.getElementsByTagName("img");//图片数组
	this.index = 0;
...
imgSwitch.prototype={
	...
	createMask:function(){
		var imgMask = document.createElement("DIV");//创建标签元素
		imgMask.id="imgMask";
		this._mask = imgMask;
		var ranges=[],masks=[],range;
		for(var i = 0;i<32;i++){
			range = document.createElement("DIV");//创建二级标签元素
		    range.className="range";
			masks.push(range.appendChild(document.createElement("DIV")));//三级标签元素，嵌套
